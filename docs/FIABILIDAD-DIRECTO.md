...
| [§5](#5-n4--alto--cada-cambio-de-parche-produce-dos-clics), [§7](#7-crash-bloques-mayores-de-lo-declarado--confirma-a4--medido), [§8](#8-silencio-por-debajo-de-32-khz--confirma-a1--medido) | Simulador de host | Reproducir literalmente las líneas 382–513 de `PluginProcessor.cpp` (cálculo de `renderBufferFrames`, `samplesError`, guardas, `resample_open`/`resample_process`) alimentando un seno; compilar contra `rdpiano_juce/Source/resample/*.c`. Contar salida por bloque, muestras a cero y desbordes. |
