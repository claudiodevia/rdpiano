...
namespace
{
  ...
  const char *const kParamIds[] = {
      "volume",         "chorusEnabled", "chorusRate",   "chorusDepth",
      "tremoloEnabled", "tremoloRate",   "tremoloDepth", "efxEnabled",
      "efxPhaserRate",  "efxPhaserDepth"};
...
TEST_SUITE(plugin_params_declared)
{
  ...
  for (int i = 0; i < kNumParams; i++)
    checks.add(std::string("existe ") + kParamIds[i],
               findParam(p, kParamIds[i]) != nullptr,
...
TEST_SUITE(plugin_state_roundtrip)
{
  ...
  const float wanted[kNumParams] = {0.37f, 0.0f, 11.0f, 2.0f,  1.0f,
                                    9.0f,  3.0f, 1.0f,  0.65f, 0.15f};
  ...
  checks.add("el preset no está vacío", state.getSize() > 0,
             check_fmt("%d bytes", (int)state.getSize()));
  ...
  for (int i = 0; i < kNumParams; i++)
    checks.add(std::string("vuelve ") + kParamIds[i],
               std::abs(after.values[i] - before.values[i]) < 1e-5f,
               check_fmt("guardado %g, restaurado %g", (double)before.values[i],
                         (double)after.values[i]));
...
TEST_SUITE(plugin_state_missing_attributes)
{
  ...
  for (int i = 0; i < kNumParams; i++)
    checks.add(std::string("de fábrica ") + kParamIds[i],
               std::abs(after.values[i] - factory.values[i]) < 1e-5f,
               check_fmt("fábrica %g, restaurado %g", (double)factory.values[i],
                         (double)after.values[i]));
...
TEST_SUITE(plugin_state_garbage)
{
  ...
  for (int i = 0; i < kNumParams; i++)
    checks.add(std::string("intacto ") + kParamIds[i],
               std::abs(after.values[i] - before.values[i]) < 1e-5f,
               check_fmt("antes %g, después %g", (double)before.values[i],
                         (double)after.values[i]));
...
TEST_SUITE(plugin_programs)
{
  ...
  for (int i = 0; i < NUM_PATCHES; i++)
    checks.add(check_fmt("nombre del programa %d", i),
               p.getProgramName(i) == juce::String(patchNames[i]),
               check_fmt("\"%s\", esperado \"%s\"",
                         p.getProgramName(i).toRawUTF8(), patchNames[i]));

  for (int i = 0; i < NUM_PATCHES; i++)
  {
    p.setCurrentProgram(i);
    checks.add(check_fmt("el motor sigue al programa %d", i),
               p.engine->patch() == i,
               check_fmt("motor %d, plugin %d", p.engine->patch(),
                         p.getCurrentProgram()));
  }
