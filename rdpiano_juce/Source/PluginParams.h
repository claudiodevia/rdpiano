...
inline constexpr RdParamSpec rdParamSpecs[kNumRdParams] = {
    ...
    {"chorusRate", "Chorus Rate", RdParamSpec::Int, 0.0f, 14.0f, (float)kEngineDefaults.chorusRate},
    {"chorusDepth", "Chorus Depth", RdParamSpec::Int, 0.0f, 14.0f, (float)kEngineDefaults.chorusDepth},
    {"tremoloEnabled", "Tremolo Enabled", RdParamSpec::Bool, 0.0f, 1.0f, kEngineDefaults.tremoloEnabled ? 1.0f : 0.0f},
    {"tremoloRate", "Tremolo Rate", RdParamSpec::Int, 0.0f, 14.0f, (float)kEngineDefaults.tremoloRate},
    {"tremoloDepth", "Tremolo Depth", RdParamSpec::Int, 0.0f, 14.0f, (float)kEngineDefaults.tremoloDepth},
    {"efxEnabled", "EFX Enabled", RdParamSpec::Bool, 0.0f, 1.0f, kEngineDefaults.efxEnabled ? 1.0f : 0.0f},
    {"efxPhaserRate", "Phaser Rate", RdParamSpec::Float, 0.0f, 1.0f, kEngineDefaults.efxPhaserRate},
...
namespace rd_params_detail
{
    ...
    {
        for (int i = 0; i < kNumRdParams; i++)
            if (rdParamSpecs[i].id == nullptr || rdParamSpecs[i].id[0] == '\0' || rdParamSpecs[i].name == nullptr ||
                rdParamSpecs[i].name[0] == '\0')
                return false;
        return true;
    }
} // namespace rd_params_detail
