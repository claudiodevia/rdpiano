...
static std::vector<float> impulse_response_spaced(int32_t rate, int32_t depth,
                                                  int n, bool sweep)
{
...
static std::vector<float> impulse_response_phaser(int32_t rate, int32_t depth,
                                                  int n, bool sweep)
{
...
TEST_SUITE(lsp_tables)
{
  ...
  static_assert(sizeof(spaceDRateTable) == sizeof(phaserRateTable),
                "las dos tablas de rate tienen que ser la misma");
  ...
  for (int i = 1; i < 128; i++)
  {
    if (spaceDRateTable[i] < spaceDRateTable[i - 1])
      rateMonotonic = false;
    if (spaceDDepthTable[i] < spaceDDepthTable[i - 1] ||
        phaserDepthTable[i] < phaserDepthTable[i - 1])
...
TEST_SUITE(lsp_spaced)
{
  ...
  std::vector<float> impulseDefault =
      impulse_response_spaced(spaceDRateTable[8], spaceDDepthTable[0x7f], N,
                              false);
  CHECK_HASH("spaced-impulso-por-defecto", hash_of(impulseDefault),
             0x2fc294a596910eb9ull);

  // Los valores que pone el plugin con chorusRate=5 y chorusDepth=14.
  std::vector<float> impulsePlugin = impulse_response_spaced(
      spaceDRateFromMs(1000.0f / 450 / 4.0f), spaceDDepth(14 / 15.0f), N,
      false);
  CHECK_HASH("spaced-impulso-plugin", hash_of(impulsePlugin),
             0x5c472a9f73a283f7ull);

  std::vector<float> sweep =
      impulse_response_spaced(spaceDRateTable[64], spaceDDepthTable[64], N,
                              true);
...
TEST_SUITE(lsp_phaser)
{
  ...
  std::vector<float> impulseDefault = impulse_response_phaser(
      phaserRateTable[16], phaserDepthTable[64], N, false);
  CHECK_HASH("phaser-impulso-por-defecto", hash_of(impulseDefault),
             0xb4b1594bb74b9afbull);

  // Los valores que pone el plugin con efxPhaserRate=0.4 y Depth=0.8.
  std::vector<float> impulsePlugin = impulse_response_phaser(
      phaserRateTable[(int)(0.4f * 0x7f)], phaserDepthTable[(int)(0.8f * 0x7f)],
      N, false);
  CHECK_HASH("phaser-impulso-plugin", hash_of(impulsePlugin),
             0xd9f8cef230ceea54ull);

  std::vector<float> sweep = impulse_response_phaser(
      phaserRateTable[100], phaserDepthTable[127], N, true);
  CHECK_HASH("phaser-barrido", hash_of(sweep), 0x5c71e1d554c6bdc9ull);

  std::vector<float> again = impulse_response_phaser(
      phaserRateTable[16], phaserDepthTable[64], N, false);
  CHECK_EQ(hash_of(again), hash_of(impulseDefault));
