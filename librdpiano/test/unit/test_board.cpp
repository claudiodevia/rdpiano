...
namespace
{
  ...
  struct Fixture
  {
    ...
    Fixture()
    {
      ...
      ok = !ic5.empty() && !ic6.empty() && !ic7.empty() && !ic18.empty() &&
           !prog.empty();
    }
  };
...
TEST_SUITE(board_memory_map)
{
  ...
  FakeCpu cpu;
  RdBoard board(f.ic5.data(), f.ic6.data(), f.ic7.data(), f.prog.data(),
                f.ic18.data());
  board.attach(&cpu);
...
TEST_SUITE(board_comm_ports)
{
  ...
  for (u32 pc : handshake)
  {
    ...
    board.commandPort().programChange(5);
    cpu.pc = pc;
    checks.add(check_fmt("puerto 1 entrega en %04x", pc),
               board.read(0x0002) == 0x35, check_fmt("PC %04x", pc));
    checks.add(check_fmt("puerto 1 consume en %04x", pc),
               board.commandPort().queue().empty(), "la cola no se vació");
  }
