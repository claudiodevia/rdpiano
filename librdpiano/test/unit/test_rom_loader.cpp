...
template <typename F>
static void check_bijection(CheckRun &checks, const char *name, size_t n, F f)
{
  ...
  checks.add(std::string(name) + " sin colisiones", collisions == 0,
             check_fmt("%zu colisión(es)", collisions));
}
...
TEST_SUITE(rom_loader_bijections)
{
  ...
  check_bijection(checks, "data_cpub", 0x100,
                  [](u32 d)
                  { return unscramble_data_cpub((u8)d); });
  check_bijection(checks, "data_wave", 0x100,
                  [](u32 d)
                  { return unscramble_data_wave((u8)d); });
...
TEST_SUITE(rom_loader_params_pages)
{
  ...
  for (int patch = 0; patch < NUM_PATCHES; patch++)
  {
    ...
    CHECK_HASH(patchNames[patch], fnv1a(out.data(), out.size()),
               PARAMS_ROM_HASHES[patch]);
    ...
    checks.add(std::string("cabecera ") + patchNames[patch],
               out[0] == 0x01 && out[1] == ((target >> 8) & 0xff) &&
                   out[2] == (target & 0xff),
               check_fmt("%02x %02x %02x, esperado 01 %02x %02x", out[0], out[1],
                         out[2], (unsigned)((target >> 8) & 0xff),
                         (unsigned)(target & 0xff)));

    // El target cae dentro de la ventana que la CPU ve mapeada en 0x4000.
    checks.add(std::string("target en ventana ") + patchNames[patch],
               target >= 0x4000 && target < 0xc000,
