...
TEST_SUITE(sa_tables_hashes)
{
  ...
  CHECK_HASH("phase_exp", fnv1a(t.phase_exp, sizeof t.phase_exp),
             PHASE_EXP_HASH);
  CHECK_HASH("samples_exp", fnv1a(t.samples_exp, sizeof t.samples_exp),
             SAMPLES_EXP_HASH);
}

// La instancia compartida tiene que ser la misma para todo el mundo: es lo que
// ahorra los 320 KB y los ~16 ms por SoundChip.
TEST_SUITE(sa_tables_shared)
{
  CHECK(&sa_tables() == &sa_tables());
}
