...
namespace
{
    ...
    std::vector<Vector> load_vectors(std::string *error)
    {
        ...
        while (fgets(line, sizeof line, f))
        {
            ...
            int got = sscanf(line,
                             ...
                             "%x %x %d %d "
                             "%x %d %x %d %d %d",
                             &sub, &env, &pitch, &loop, &high, &dest, &speed, &off, &f0, &f1, &volume, &irq, &envOut,
                             &subOut, &addr, &sel, &hi, &wexp, &expSign, &wdelta, &deltaSign, &added, &raw);
            if (got != 23)
            {
    ...
    void report(CheckRun &checks, const char *name, size_t bad, size_t total, const FirstFailure &first)
    {
        checks.add(name, bad == 0,
                   check_fmt("%zu de %zu vector(es); primero en la línea %d: %s", bad, total, first.line,
                             first.detail.c_str()));
    }
...
TEST_SUITE(sound_chip_blocks_vectors)
{
    ...
    checks.add("vectores cargados", !vectors.empty(), error.empty() ? "fichero vacío" : error);
    if (vectors.empty())
        return;
    ...
    for (const Vector &v : vectors)
    {
        ...
        if (ic19.volume != v.volume || ic19.irq != v.irq || part.env_value != v.env_value_out)
        {
            badIc19++;
            firstIc19.note(v.line, check_fmt("volume %08x/%08x irq %d/%d env %08x/%08x", ic19.volume, v.volume,
                                             ic19.irq ? 1 : 0, v.irq ? 1 : 0, part.env_value, v.env_value_out));
        }

        // --- IC9: fase, dirección de wave ROM y los dos selectores.
        Ic9Out ic9 = tick_ic9(part, flags, tables);
        if (part.sub_phase != v.sub_phase_out || ic9.waverom_addr != v.waverom_addr ||
            ic9.sel_sample_type != v.sel_sample_type || ic9.phase_hi != v.phase_hi)
        {
            ...
            firstIc9.note(v.line, check_fmt("phase %08x/%08x addr %08x/%08x sel %d/%d "
                                            "hi %d/%d",
                                            part.sub_phase, v.sub_phase_out, ic9.waverom_addr, v.waverom_addr,
                                            ic9.sel_sample_type ? 1 : 0, v.sel_sample_type ? 1 : 0,
        ...
        s32 sample = tick_ic8(part, ic19, ic9, v.waverom_exp, v.exp_sign, v.waverom_delta, v.delta_sign, tables);
        ...
        {
            badSilencing++;
            firstSilencing.note(v.line, check_fmt("%d, esperado %d", added, v.sample_added));
        }
    }
...
TEST_SUITE(sound_chip_blocks_coverage)
{
    ...
    size_t irq = 0, sel = 0, hi = 0, noFlags0 = 0, noFlags1 = 0, silenced = 0, speedInv = 0, phaseWrap = 0;
