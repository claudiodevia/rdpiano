...
static void check_bytes(CheckRun &checks, const char *name,
                        ...
                        const std::vector<u8> &want)
{
  ...
  for (u8 b : want)
    w += check_fmt("%02x ", b);
  checks.add(name, got == want,
             check_fmt("emitido [%s], esperado [%s]", g.c_str(), w.c_str()));
}
...
TEST_SUITE(command_port_master_tune)
{
  ...
  for (const Case &c : cases)
  {
    ...
    checks.add(check_fmt("tune %6d", c.tune),
               got.msb == c.msb && got.lsb == c.lsb,
               check_fmt("%02x %02x, esperado %02x %02x", got.msb, got.lsb,
                         c.msb, c.lsb));
  }
...
TEST_SUITE(command_port_boot_sequence)
{
  ...
  port.programChange(0);
  port.masterTune(0);
  check_bytes(checks, "arranque, antes del margen", drain(port),
              {0x30, 0xE0, 0x00, 0x00});

  // Epílogo: recarga del parche, después.
  port.reloadPatch();
  check_bytes(checks, "arranque, después del margen", drain(port),
              {0x31, 0x30});

  // Con afinación distinta de cero cambian dos bytes y solo dos.
  port.programChange(0);
  port.masterTune(-16384);
  check_bytes(checks, "arranque afinado", drain(port),
              {0x30, 0xE0, 0x7f, 0x68});
}
