...
static Stereo render_blocks(RdPianoEngine *e, int total, const int *blocks,
                            int numBlocks)
{
  Stereo out;
  out.l.reserve(total);
...
TEST_SUITE(engine_host_rates)
{
  ...
  for (double hostRate : rates)
  {
    for (int patch : patches)
    {
      ...
      CHECK_MSG(out.l.size() == (size_t)total, "%.0f Hz parche %d: %zu de %d",
                hostRate, patch, out.l.size(), total);
      CHECK_MSG(all_finite(out.l) && all_finite(out.r), "%.0f Hz parche %d",
                hostRate, patch);
      ...
      CHECK_MSG(level > 1e-4, "%.0f Hz parche %d: rms %.3g", hostRate, patch,
                level);

      // Y sin quedarse sin muestras: ni bloques vacíos ni desbordes.
      CHECK_MSG(e->stats.tooFewFrames == 0, "%.0f Hz parche %d: %lu", hostRate,
                patch, e->stats.tooFewFrames);
      CHECK_MSG(e->stats.tooManyFrames == 0, "%.0f Hz parche %d: %lu", hostRate,
                patch, e->stats.tooManyFrames);
...
TEST_SUITE(engine_no_alloc_in_render)
{
  ...
  checks.add("render-sin-operator-new", g_allocCount == 0,
             check_fmt("%ld reservas en 32 bloques", g_allocCount));
...
TEST_SUITE(engine_patch_change)
{
  ...
  checks.add("parche-sin-clic", worstStep < 0.25,
             check_fmt("mayor salto entre muestras %.4f", worstStep));
...
TEST_SUITE(engine_midi_timing)
{
  ...
  checks.add("midi-silencio-antes", before < after / 10.0,
             check_fmt("rms antes %.5g, después %.5g", before, after));
  checks.add("midi-suena-despues", after > 1e-4,
             check_fmt("rms después %.5g", after));
  ...
  checks.add("midi-empieza-en-su-frame", firstAudible + 1 >= (size_t)onset,
             check_fmt("primera muestra audible %zu, pedida en %d",
                       firstAudible, onset));
